...
def backfill_yields_history(history: list) -> list:
    ...
    for d in all_dates:
        ...
        # SOFR 曲线形态（Compounded Bootstrap 隐含远期利差）
        s1m = all_obs[d].get("sofr_1m")
        s3m = all_obs[d].get("sofr")
        s6m = all_obs[d].get("sofr_6m")
        if s1m is not None and s3m is not None:
            fwd_1x3 = (s3m * 3 - s1m * 1) / 2
            all_obs[d]["sofr_fwd1x3"] = round((fwd_1x3 - s3m) * 100, 1)
        if s3m is not None and s6m is not None:
            fwd_3x6 = (s6m * 6 - s3m * 3) / 3
            all_obs[d]["sofr_fwd3x6"] = round((fwd_3x6 - s3m) * 100, 1)
...
def fetch_macro_fundamentals() -> dict:
    """
    抓取分子端经济活跃度指标（月频，取最新一期）：
      kr_export_yoy  %   韩国出口同比 (FRED XTEXVA01KRM664S)
      cn_pmi_mfg     pt  中国制造业PMI官方 (akshare)
      cn_pmi_caixin  pt  财新制造业PMI (akshare)
    """
    api_key = os.environ.get("FRED_API_KEY", "")
    result = {}
    ...
    if api_key:
        try:
            v = _fred_latest("XTEXVA01KRM664S", api_key)
            result["kr_export_yoy"] = round(v, 2)
        except Exception as e:
            print(f"   XTEXVA01KRM664S ERROR: {e}")

    # ── 中国制造业PMI（akshare 官方，macro_china_pmi 按最新在首行）───
    try:
        df = ak.macro_china_pmi()          # 最新一行在 iloc[0]，col[1]=制造业指数
        val = float(df.iloc[0, 1])
        result["cn_pmi_mfg"] = round(val, 1)
    except Exception as e:
        print(f"   cn_pmi_mfg ERROR: {e}")

    # ── 财新制造业PMI（akshare index_pmi_man_cx）────────────────────
    try:
        df = ak.index_pmi_man_cx()
        df = df.sort_values(df.columns[0])  # 升序，最新在末行
        val_col = df.columns[1]             # 制造业PMI
        df = df.dropna(subset=[val_col])
        if not df.empty:
            result["cn_pmi_caixin"] = round(float(df.iloc[-1][val_col]), 1)
    except Exception as e:
        print(f"   cn_pmi_caixin ERROR: {e}")

    return result


def backfill_macro_fundamentals(history: list) -> list:
    """
    回填分子端历史 90 天数据，仅在 kr_export_yoy 不足 60 条时触发。
    韩国出口：FRED _fred_series_90d（月频，forward-fill）
    中国PMI：akshare 历史接口（月频，forward-fill）
    """
    has_kr = sum(1 for r in history if r.get("kr_export_yoy") is not None)
    if has_kr >= 60:
        return history
    ...
    if api_key:
        try:
            obs = _fred_series_90d("XTEXVA01KRM664S", api_key, days=400)
            for d, v in obs.items():
                all_obs.setdefault(d, {})["kr_export_yoy"] = round(v, 2)
            print(f"   XTEXVA01KRM664S: {len(obs)} 条")
        except Exception as e:
            print(f"   XTEXVA01KRM664S ERROR: {e}")

    # ── 中国PMI官方（macro_china_pmi，按最新在首行）───────────────────
    try:
        import re, calendar as _cal
        df = ak.macro_china_pmi()           # col[0]=年月, col[1]=制造业指数，最新在首行
        count_mfg = 0
        for _, row in df.iterrows():
            m = re.match(r'(\d{4})\D+?(\d{1,2})\D', str(row.iloc[0]))
            if m:
                yr, mo = int(m.group(1)), int(m.group(2))
                last_day = _cal.monthrange(yr, mo)[1]
                d = f"{yr:04d}-{mo:02d}-{last_day:02d}"
                try:
                    all_obs.setdefault(d, {})["cn_pmi_mfg"] = round(float(row.iloc[1]), 1)
                    count_mfg += 1
                except (ValueError, TypeError):
                    pass
            if count_mfg >= 18:
                break
        print(f"   cn_pmi_mfg: {count_mfg} 条")
    except Exception as e:
        print(f"   cn_pmi_mfg ERROR: {e}")

    # ── 财新制造业PMI（index_pmi_man_cx）──────────────────────────────
    try:
        df = ak.index_pmi_man_cx()
        df = df.sort_values(df.columns[0])  # 升序，最新末行
        val_col = df.columns[1]
        df = df.dropna(subset=[val_col]).tail(18)
        for _, row in df.iterrows():
            d = str(row[df.columns[0]])[:10]
            try:
                all_obs.setdefault(d, {})["cn_pmi_caixin"] = round(float(row[val_col]), 1)
            except (ValueError, TypeError):
                pass
        print(f"   cn_pmi_caixin: {len(df)} 条")
    except Exception as e:
        print(f"   cn_pmi_caixin ERROR: {e}")

    # ── Forward-fill 月频到所有日期 ──────────────────────────────
    all_dates = sorted(set(list(all_obs.keys()) + [r["date"] for r in history]))

    def _ffill_raw(raw_field: str) -> dict:
        out, last = {}, None
        for d in all_dates:
            if d in all_obs and all_obs[d].get(raw_field) is not None:
                last = all_obs[d][raw_field]
            out[d] = last
        return out

    kr_ff  = _ffill_raw("kr_export_yoy")
    mfg_ff = _ffill_raw("cn_pmi_mfg")
    cx_ff  = _ffill_raw("cn_pmi_caixin")
    ...
    for d in all_dates:
        if d not in existing:
            continue
        rec = history[existing[d]]
        for field, ff in [("kr_export_yoy", kr_ff), ("cn_pmi_mfg", mfg_ff), ("cn_pmi_caixin", cx_ff)]:
            if ff.get(d) is not None and rec.get(field) is None:
                rec[field] = ff[d]
                updated += 1
    ...
    print(f"  分子端回填完成：补字段 {updated} 处，共 {len(history)} 条")
    return history
...
def update_index_html(history: list) -> bool:
    ...
    srf_arr         = _ffill("srf")
    tga_arr         = _ffill("tga")
    kr_export_arr  = _ffill("kr_export_yoy")
    cn_pmi_mfg_arr = _ffill("cn_pmi_mfg")
    cn_pmi_cx_arr  = _ffill("cn_pmi_caixin")
    # ── ⑥ 外汇信号 ──────────────────────────────────────────────────
    ...
    new_block = (
        ...
        f"const SRF_ARR={json.dumps(srf_arr)};\n"
        f"const TGA_ARR={json.dumps(tga_arr)};\n"
        f"const KR_EXPORT_YOY={json.dumps(kr_export_arr)};\n"
        f"const CN_PMI_MFG={json.dumps(cn_pmi_mfg_arr)};\n"
        f"const CN_PMI_CAIXIN={json.dumps(cn_pmi_cx_arr)};\n"
        ...
        f"onrrp:[...ONRRP],srf:[...SRF_ARR],tga:[...TGA_ARR],"
        ...
        f"krExport:[...KR_EXPORT_YOY],cnPmiMfg:[...CN_PMI_MFG],cnPmiCaixin:[...CN_PMI_CAIXIN]}};"
    )
...
def _compute_macro_state(today: dict) -> dict:
    ...
    # 维度6：分子端经济活跃度（权重0.25）
    kr_exp = today.get("kr_export_yoy")
    cn_pmi = today.get("cn_pmi_mfg")
    if kr_exp is not None:
        ms = (2.0 if kr_exp > 10 else
              1.0 if kr_exp > 5 else
              0.0 if kr_exp > 0 else
              -1.0 if kr_exp > -5 else -2.0)
        if cn_pmi is not None:
            ms += 0.5 if cn_pmi > 51 else (-0.5 if cn_pmi < 49 else 0.0)
        score += max(-2.0, min(2.0, ms)) * 0.25
...
def _push_feishu_unified_inner(history: list, webhook_url: str) -> None:
    ...
    # ── §0 分子端经济活跃度 ───────────────────────────────────────────
    kr_exp    = rec.get("kr_export_yoy")
    cn_pmi_m  = rec.get("cn_pmi_mfg")
    cn_pmi_cx = rec.get("cn_pmi_caixin")

    if kr_exp is not None:
        if   kr_exp > 5:   mf_icon, mf_sig = "🟢", "全球需求扩张"
        elif kr_exp > 0:   mf_icon, mf_sig = "🟡", "需求温和"
        elif kr_exp > -5:  mf_icon, mf_sig = "🟠", "需求收缩预警"
        else:              mf_icon, mf_sig = "🔴", "需求明显萎缩"
    else:
        mf_icon, mf_sig = "⚪", "数据待接入"

    kr_str = f"{kr_exp:+.1f}%" if kr_exp is not None else "--"
    mfg_str = f"{cn_pmi_m:.1f}" if cn_pmi_m is not None else "--"
    cx_str  = f"{cn_pmi_cx:.1f}" if cn_pmi_cx is not None else "--"

    sec_macro_fund = (
        f"**【分子端·经济活跃度】**\n"
        f"韩国出口: {kr_str}(同比) ｜ 中国PMI: {mfg_str} ｜ 财新PMI: {cx_str}\n"
        f"状态：{mf_icon} {mf_sig}"
    )
    ...
    if f1x3 is not None and f3x6 is not None:
        ...
        if shape < -10:
            curve_label = "Back·市场预期显著降息"
        elif shape < 0:
            curve_label = "Back·市场预期小幅降息"
        elif shape < 10:
            curve_label = "Contango·市场预期平稳"
        else:
            curve_label = "Contango·市场预期收紧"
        curve_line = (
            f"曲线结构：1x3 {f1x3:+.1f}bp / 3x6 {f3x6:+.1f}bp / "
            f"差值 {shape:+.1f}bp [{curve_label}]"
        )
    else:
    ...
    card = {
        ...
        "elements": [
            {"tag": "div", "text": {"tag": "lark_md", "content": sec_macro_fund}},
            {"tag": "hr"},
...
def main():
    ...
    # SOFR 曲线形态（Compounded Bootstrap 隐含远期利差）
    s3m = record.get("sofr")      # SOFR90DAYAVG %
    s1m = record.get("sofr_1m")   # SOFR30DAYAVG %
    s6m = record.get("sofr_6m")   # SOFR180DAYAVG %
    if s1m is not None and s3m is not None:
        fwd_1x3 = (s3m * 3 - s1m * 1) / 2
        record["sofr_fwd1x3"] = round((fwd_1x3 - s3m) * 100, 1)
        record["sofr_fwd1x3_abs"] = round(fwd_1x3, 4)
    if s3m is not None and s6m is not None:
        fwd_3x6 = (s6m * 6 - s3m * 3) / 3
        record["sofr_fwd3x6"] = round((fwd_3x6 - s3m) * 100, 1)
        record["sofr_fwd3x6_abs"] = round(fwd_3x6, 4)
    if "sofr_fwd1x3" in record and "sofr_fwd3x6" in record:
        record["sofr_curve_shape"] = round(record["sofr_fwd1x3"] - record["sofr_fwd3x6"], 1)
    if "sofr_fwd1x3" in record and "sofr_fwd3x6" in record:
        f1 = record["sofr_fwd1x3"]
        f3 = record["sofr_fwd3x6"]
        shape = record.get("sofr_curve_shape", 0)
        structure = "Backwardation" if shape < 0 else "Contango"
        print(f"   SOFR曲线(Compounded) 1x3={f1:+.1f}bp  "
              f"3x6={f3:+.1f}bp  "
              f"形态={shape:+.1f}bp [{structure}]")
    ...
    print("\n⑫ 分子端经济活跃度（韩国出口 + 中国PMI）")
    try:
        mf = fetch_macro_fundamentals()
        for fld in ("kr_export_yoy", "cn_pmi_mfg", "cn_pmi_caixin"):
            if fld in mf:
                record[fld] = mf[fld]
        parts = []
        if mf.get("kr_export_yoy") is not None: parts.append(f"韩国出口={mf['kr_export_yoy']:+.2f}%")
        if mf.get("cn_pmi_mfg")    is not None: parts.append(f"PMI={mf['cn_pmi_mfg']:.1f}")
        if mf.get("cn_pmi_caixin") is not None: parts.append(f"财新={mf['cn_pmi_caixin']:.1f}")
        print(f"   {'  '.join(parts) if parts else '(部分字段获取失败)'}  ✓")
    except Exception as e:
        print(f"   ERROR: {e}")
        hist_tmp = load_history()
        for fld in ("kr_export_yoy", "cn_pmi_mfg", "cn_pmi_caixin"):
            last = next((r[fld] for r in reversed(hist_tmp) if r.get(fld) is not None), None)
            if last is not None:
                record[fld] = last
                print(f"   {fld} 使用上次已知值（填充）")
    ...
    history = backfill_macro_fundamentals(history)   # kr_export_yoy 不足 60 条时回填
